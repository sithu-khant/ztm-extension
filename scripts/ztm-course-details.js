...
const courseDetailsIconPosition = () => {
    ...
    const isFavoriteCoursesFeature =
        favoriteCoursesFeature?.getAttribute('style')

    const ztmCourseDetailsDivs = document.querySelectorAll(
        '#ztm-course-details'
    )
...
const courseDetails = () => {
    ...
    const observeCourseDetails = new MutationObserver(() =>
        courseDetailsIconPosition()
    )
    observeCourseDetails.observe(document.body, {
        childList: true,
        subtree: true,
    })

    // Get all the course details icon
    let ztmCourseDetailsIcons = document.querySelectorAll(
        '#ztm-course-details-icon'
    )

    ztmCourseDetailsIcons.forEach((courseDetailsIcon) => {
        // Get the course data
        const courseCard = courseDetailsIcon.closest(
            '.col-xs-12.col-sm-6.col-md-4'
        )
        const courseTitle = courseCard
            .querySelector('.course-listing-title')
            .textContent.trim()
        // Get `ztm-course-details-anchor` tag
        const ztmCourseDetailsAnchor = courseDetailsIcon.closest(
            '#ztm-course-details-anchor'
        )

        // Course title data cleaning
        const removeColonFromTitle = courseTitle.replace(/:/g, '')
        const removeBracketsFromTitle = removeColonFromTitle.replace(
            /[()]/g,
            ''
        )
        const removeYearsFromTitle = removeBracketsFromTitle.replace(
            /\b\d{4}\b/g,
            ''
        )
        ...
        fetch(jsonUrl)
            ...
            .then((responses) => {
                responses.forEach((res) => {
                    const removeColonFromResTitleArray = res.name?.replace(
                        /:/g,
                        ''
                    )
                    const removeBracketsFromResTitleArray =
                        removeColonFromResTitleArray.replace(/[()]/g, '')
                    const removeYearsFromResTitleArray =
                        removeBracketsFromResTitleArray.replace(
                            /\b\d{4}\b/g,
                            ''
                        )
                    const removeInFromResTitleArray =
                ...
                const getCourseDetailsAnchor = courseDetailsIcon.closest(
                    '#ztm-course-details-anchor'
                )
                if (!getCourseDetailsAnchor.href) {
                    const getCourseDetails = courseDetailsIcon.closest(
                        '#ztm-course-details'
                    )
...
const toggleCourseDetails = (isChecked) => {
    ...
    courseListing.forEach((course) => {
        const ztmCourseDetailsDivs = course.querySelectorAll(
            '#ztm-course-details'
        )
...
const ztmCourseDetails = () => {
    ...
    chrome.storage.onChanged.addListener((changes, namespace) => {
        if (
            namespace === 'sync' &&
            'ztmCourseDetailsCheckboxIsChecked' in changes
        ) {
            let isChecked =
                changes.ztmCourseDetailsCheckboxIsChecked.newValue || false
            courseDetails()
            toggleCourseDetails(isChecked)
        }
    })
